...
머리 근접 — 토픽 `safe-vibe/alert` (양쪽). 움직이는 박스가 사람 머리 주변에 들어오면 온다.

```json
{"event":"box_near_head","ts":"10:12:40","ts_epoch":1789374760.2,"track_id":41,"name":"box",
 "person_id":3,"side":"both","move":0.07,"speed":0.15}
```

`event` 가 `roi_dwell` / `fall_warning` / `box_near_head` 인 것만 처리한다. 머리 근접은 이
이벤트를 처리하는 스케치를 올린 보드만 울린다 — 예전 스케치는 `알 수 없는 event` 로 무시한다. 낙하 경보는 `side` 가
...
`체류` 는 두 보드 모두 체류 패턴(길게 3번)으로, `머리` 는 두 보드 모두 머리 근접 경보(짧게 6번)로
울린다. 패널 제목 옆에 브로커 연결 상태가 보인다.
